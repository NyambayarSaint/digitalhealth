...
const checkLanguage = async (queryString, req, normal) =>{
    ...
    if(normal){
        return new Promise(async(resolve, reject) => {
            if(cookies['tb-lang'] === "en"){
                const res = await fetch(`https://digitalhealthmongolia.herokuapp.com${queryString}`);
    ...
    return new Promise(async(resolve, reject)=>{
      if(cookies['tb-lang'] === "en"){
          const res = await fetch('https://digitalhealthmongolia.herokuapp.com/graphql', httpObject)
          const pageData = await res.json()
